...
public class Enemy : MonoBehaviour
{
    ...
    void OnTriggerEnter2D (Collider2D c)
    {
        ...
        // Bulletコンポーネントを取得（Bulletスクリプトを持つPlayerBulletがCollider2Dを持つBulletの親になっている）
        Bullet bullet = c.GetComponentInParent<Bullet>();
...
アニメーションを行うプロパティを登録します。`Add Curve`ボタンで**Sprite
